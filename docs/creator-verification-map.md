...
| Preserve QBR, X artifacts, Brain Sync, YouTube editing and legacy workflow IDs | Canonical advanced skill/workflow/receipt deltas reviewed; manifests, disposition audit and generated copies checked | Actual host scenarios remain unrun. Eddy is separate and unexecuted. |
...
| Reproducible archive and extracted execution | Clean 67a2ec7 candidate built twice: five identical outputs, 265 extracted plugin files matching inventory/hashes; extracted Day/Week/Month walkthroughs and separate status readbacks passed | Final artifact must include later documentation commits. Tested local tar toolchain; Linux CI not yet run. |
...
| Native installer safety and proof state | Installer tests cover removal failures, unresolved mutations, target isolation, marketplace ownership, unverified native status and locks. Exact 67a2ec7 candidate installed in isolated Codex 0.147.0; all 265 cache files matched. Minimal update probes passed on 0.147.0 and 0.144.1 | Cached installation is not host routing or OAuth. Current desktop runtime and actual Windows CI remain unproven. |
