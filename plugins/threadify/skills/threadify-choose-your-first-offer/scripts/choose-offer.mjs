...
export function artifacts(input){
  ...
  const nextStep='\n## Optional next step\n\nWould you like me to create a ChatGPT Sites landing page to test this offer, then help you deploy it and prepare it to share on Threads?\n\nNothing has been deployed or posted. Draft creation, public deployment and Threads publishing are separate choices. The page should use truthful, benefit-led offer copy, actual proof and a verified validation action, not invented results or scarcity.\n';
  return {'offer-decision.json':JSON.stringify(body,null,2)+'\n','offer-decision.md':markdown+nextStep,'validation-page.html':html,'proof-loop-map.json':JSON.stringify(map,null,2)+'\n','receipt.json':JSON.stringify(receipt,null,2)+'\n'};
}
