...
function validateSource(source) {
  exactKeys(source, [
    ...
    'returned_count', 'complete', 'cache_status', 'endpoint', 'query_scope',
    'freshness_status', 'gaps', 'retention_policy', 'evidence_ref',
  ...
  for (const key of ['cache_status', 'endpoint', 'query_scope', 'freshness_status', 'retention_policy']) {
    if (source[key] !== undefined && source[key] !== null && (typeof source[key] !== 'string' || !source[key].trim())) {
      fail(`source_${key}_invalid`);
    }
  }
  if (source.gaps !== undefined && (!Array.isArray(source.gaps)
    || source.gaps.some((gap) => typeof gap !== 'string' || !gap.trim()))) fail('source_gaps_invalid');
  return {
    ...structuredClone(source),
    cache_status: source.cache_status ?? null,
    endpoint: source.endpoint ?? null,
    query_scope: source.query_scope ?? null,
    freshness_status: source.freshness_status ?? null,
    gaps: structuredClone(source.gaps ?? []),
    retention_policy: source.retention_policy ?? null,
  };
}
...
export function runForensics(input, { visualDirectory = null } = {}) {
  ...
  return {
    ...
    receipt: {
      ...
      source_count: sources.length,
      source_receipts: sources.map((source) => ({
