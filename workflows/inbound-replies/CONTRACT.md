...
Output contains the durable `attempt.key` and exact `mcp` arguments for `send_replies`. `record-send`: `{key,receipt,eta}`. Receipts are stable provider evidence references, not local success labels.
