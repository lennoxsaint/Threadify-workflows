...
async function main() {
  ...
  if (process.argv[2] === 'market-to-pipeline') {
    const { marketToPipelineMain } = await import('../lib/market-to-pipeline-cli.mjs');
    process.stdout.write(`${JSON.stringify(marketToPipelineMain(process.argv.slice(3)), null, 2)}\n`);
    return;
  }
