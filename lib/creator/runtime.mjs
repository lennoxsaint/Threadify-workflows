...
export async function runCreatorCommand(command, { root, revision, input = {} }) {
  ...
  if (command === 'apply-browser-review') {
    browserSession = (await readState(input.session_root)).payload;
    assert(browserSession?.schema_version === 'creator-browser-review.v1' && browserSession.status === 'submitted', 'Submit the browser review first.');
    const { hash, ...body } = browserSession.submission;
    assert(hash === reviewHash(body), 'Submitted review integrity mismatch.');
  }
  ...
  const state = await updateState(root, revision, (payload) => {
    ...
    if (command === 'apply-browser-review') {
      ...
      assert(reviewHash(getPlan(data, submission.plan_id)) === browserSession.plan_hash, 'Plan changed since browser review.');
      for (const entry of submission.entries) {
        const current = getReview(data, entry.review_id);
        const card = current.cards.find((c) => c.content.id === entry.content.id);
        assert(card && reviewHash(card.content) === entry.base_hash, 'Source changed since browser review; never overwrite newer edits.');
        validateReuse(entry.content, input.reuse_proofs?.[entry.content.id] ?? current.reuse_evidence?.[entry.content.id], input.now);
        if (reviewHash(card.content.parts) !== reviewHash(entry.content.parts)) data.feedback.push(createFeedback({
          id: `browser-${submission.session_id}-${entry.content.id}`, account_id: entry.content.account_id,
          review_id: entry.review_id, card_id: entry.content.id, original_parts: card.content.parts,
          final_parts: entry.content.parts, instruction: 'Exact manual browser edit', rating: null, now: submission.at }));
        let next = replaceCard(current, entry.content);
        if (input.reuse_proofs?.[entry.content.id]) next.reuse_evidence = { ...next.reuse_evidence, [entry.content.id]: input.reuse_proofs[entry.content.id] };
        next = approveReview(next, displayReview(next, [entry.content.id]), {
          at: submission.at, evidence_ref: `browser-submit:${submission.session_id}:${submission.hash}` });
        data.reviews[data.reviews.indexOf(current)] = next;
      }
      result = { action: 'browser_approval_applied', submission_hash: submission.hash,
        cards: submission.entries.length, provider_writes_performed: false };
    } else if (LEARNING_WRITES.has(command)) {
