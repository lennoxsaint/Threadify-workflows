...
const SIGNAL_KINDS = new Set([
  ...
  'company_fit', 'work_contact_verification', 'lawful_basis',
]);
...
function actionBinding(runId, prospect) {
  return {
    schema_version: 1,
    run_id: runId,
    ...
    destination: prospect.contact.destination,
    exact_text: prospect.action.exact_text,
    evidence_ids: [...prospect.action.evidence_ids].sort(),
  };
}
...
function validateContact(contact) {
  exactKeys(contact, ['channel', 'destination', 'work_contact_verified', 'verified_at', 'verifier', 'permission_signal_id',
    'lawful_basis_signal_id', 'jurisdiction', 'sender_identity', 'sender_contact', 'unsubscribe_supported',
  ...
  for (const key of ['jurisdiction', 'sender_identity', 'sender_contact']) text(contact[key], `contact_${key}`);
}
...
function readiness(prospect, signals, observedAt) {
  ...
  if (prospect.lane === 'warm') {
    ...
    if (!lawfulBasis || lawfulBasis.kind !== 'lawful_basis' || !lawfulBasis.relevant_to_offer
...
function validateCampaign(campaign, themes, identifiers) {
  ...
  for (const post of campaign.posts) {
    ...
    if (!Array.isArray(post.evidence_theme_ids) || !post.evidence_theme_ids.length) fail('campaign_post_evidence_missing');
    for (const themeId of post.evidence_theme_ids) if (!themes.has(themeId)) fail(`campaign_post_unknown_theme:${themeId}`);
    const normalized = post.exact_text.toLowerCase();
    for (const identifier of identifiers) {
      if (identifier.length >= 3 && normalized.includes(identifier.toLowerCase())) fail(`campaign_private_identifier:${post.post_id}`);
    }
  }
}
...
export function buildMarketPipeline(input) {
  exactKeys(input, ['record_type', 'schema_version', 'run_id', 'observed_at', 'offer', 'market_scope', 'account',
    'sources', 'signals', 'themes', 'prospects', 'campaign'], 'input');
  ...
  const identifiers = input.prospects.flatMap((prospect) => [prospect.display_name, prospect.company_name, prospect.contact.destination]);
  validateCampaign(input.campaign, themes, identifiers);
  ...
  const pipeline = selected.map(({ prospect, readiness: gate }) => {
    const binding = actionBinding(input.run_id, prospect);
    ...
    return {
      ...
      action: {
        ...
        action_hash: hash,
        idempotency_key: sha256({ run_id: input.run_id, action_id: prospect.action.action_id, action_hash: hash }),
  ...
  const receipt = {
    ...
    action_bindings: pipeline.map((entry) => ({ action_id: entry.action.action_id, action_hash: entry.action.action_hash,
...
export function editMarketAction(state, { action_id, exact_text }) {
  ...
  const hash = sha256(actionBinding(next.run_id, prospect));
...
export function beginMarketActionAttempt(state, { action_id, action_hash, attempt_id, started_at }) {
  const next = structuredClone(state);
  const prospect = findAction(next, action_id);
  ...
  prospect.action.attempts.push({ attempt_id, action_hash, started_at, status: 'pending', checked_at: null,
    provider_ref_sha256: null, evidence_ref_sha256: null });
...
export function reconcileMarketActionAttempt(state, { action_id, attempt_id, status, checked_at, provider_ref, evidence_ref }) {
  ...
  if (prospect.action.state !== 'attempt_pending') fail('no_pending_attempt_to_reconcile');
  const attempt = prospect.action.attempts.at(-1);
  if (!attempt || attempt.attempt_id !== attempt_id || attempt.status !== 'pending') fail('pending_attempt_mismatch');
...
export function writeMarketPipelineArtifacts(state, outputDirectory) {
  ...
  const publicReceipt = state.receipts.find((entry) => entry.record_type === 'MarketToPipelinePublicReceiptV1');
