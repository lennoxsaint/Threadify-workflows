...
const RESPONSIVE_CSS = '<style>*,*::before,*::after{box-sizing:border-box}body{width:100%;overflow-wrap:anywhere}.grid,.lane,.editor{min-width:0}a{overflow-wrap:anywhere}@media(max-width:750px){.grid{grid-template-columns:minmax(0,1fr)}}</style>';
...
export function buildLeadDesk(input) {
  ...
  return { ...desk, markdown: renderMarkdown(desk), html: renderHtml(desk).replace('</head>', `${RESPONSIVE_CSS}</head>`) };
}
